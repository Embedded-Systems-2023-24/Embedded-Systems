...
void compute_matrices (
	...
	int max_index_buf = 0;
	int max_value = 0;
	ap_int<3> string1[N] ={P};
#pragma HLS ARRAY_PARTITION variable=string1 dim=0 factor=2 cyclic
	ap_int<3> string2[M+2*(N-1)] ={P};
#pragma HLS ARRAY_PARTITION variable=string2 dim=0 factor=2 cyclic
	int current_diag[N] = {0};
#pragma HLS ARRAY_PARTITION variable=current_diag dim=0 complete
	int up_diag[N] = {0};
#pragma HLS ARRAY_PARTITION variable=up_diag dim=0 complete
	int upper_diag[N] = {0};
#pragma HLS ARRAY_PARTITION variable=upper_diag dim=0 complete
	ap_int<3> direction_diag[N];
#pragma HLS ARRAY_PARTITION variable=direction_diag dim=0 complete
