...
void compute_matrices (ap_int<3> string1_mem[N], ap_int<3> string2_mem[M+2*(N-1)], int max_index[0], ap_int<3> direction_matrix[(M+2*(N-1))*N], int n, int m);
...
/* Create the two input strings by calling a random number generator */
	...
	compute_matrices(query_hw, database_hw, max_index, direction_matrix, N, M);
